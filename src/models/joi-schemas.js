...
export const UserCredentialsSpec = {
  ...
  export const DetailSpec = Joi.object({
    ...
    latitude: Joi.number().min(-90).max(90).when('postcode', { is: 'null', then: Joi.required(), otherwise: Joi.allow(null) }),
    longitude: Joi.number().min(-180).max(180).when('postcode', { is: 'null', then: Joi.required(), otherwise: Joi.allow(null) }),
    ...
    export const LocationSpec = Joi.object({
      title: Joi.string().required(),
      isSetLocation: Joi.boolean().optional(), 
    });
